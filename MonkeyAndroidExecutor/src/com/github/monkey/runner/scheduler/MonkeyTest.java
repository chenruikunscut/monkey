...
//        mTestId = UUID.randomUUID().toString();
    ...
    public class MonkeyTestItemExecutor extends Thread {
        ...
        private LogOutputReceiver mActivityLogcatLogOutputReceiver;
        ...
        void startActivityLogcatThread() {
            try {
                String f = LocationHelper.getActivityLogcatLogLocation(mTestId);
                mActivityLogcatLogOutputReceiver = new LogOutputReceiver(f);
                Thread t = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        mDevice.activityLogcat(mActivityLogcatLogOutputReceiver);
                        mLogcatLogOutputReceiver.cancel();
                    }
                });
                t.start();
        ...
        public void run() {
            try {
                ...
                // wait for device
                log("Wating for device 20 seconds");
