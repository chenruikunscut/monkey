...
public class MonkeyTestDevice {
    ...
    public boolean monkey(String command, IShellOutputReceiver receiver) {
        boolean result = false;
        try {
            mDevice.executeShellCommand(command, receiver,30000);
            result = true;
    ...
    public boolean logcat(IShellOutputReceiver receiver) {
        boolean result = false;
        try {
            mDevice.executeShellCommand("logcat -c",receiver);
            mDevice.executeShellCommand("logcat -v time *:V", receiver,
                    30000);
            result = true;
        } catch (ShellCommandUnresponsiveException mttor) {
            result = true;
    ...
    public boolean activityLogcat(IShellOutputReceiver receiver){
        boolean result = false;
        try {
            mDevice.executeShellCommand("logcat -c",receiver);
            mDevice.executeShellCommand("logcat -v time ActivityManager:I *:S | grep \"com.jd.wxsq.app\"",receiver,20000);
            result = true;
        } catch (ShellCommandUnresponsiveException mttor) {
            result = true;
        } catch (TimeoutException e) {
            Console.printLogMessage(mDevice.getSerialNumber(),"ExecuteCommand Timeout logcat -v time ActivityManager:I *:S");
            e.printStackTrace();
            result = false;
